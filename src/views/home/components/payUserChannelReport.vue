<template>
    <div id="payUserChannelReport" style="width:100%;height:100%;">
    </div>
</template>
<script>
        ...
        this.$http.post(url, null).then(function(res){
           if(!!res && res.resultCode=='0'){
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    rpData.push(entity);
                    legendData.push(list[i].channel);
                }
                let option = {
                tooltip: {
                    trigger: 'item',
                    formatter: "{b} <br/>"+ref.$t('Totalmembership')+"{c} ({d}%)"
                },
                legend: {
                    type: 'scroll',
                    orient: 'vertical',
                ...
                series: [
                    {
                        ...
                        label: {
                            normal: {
                               // formatter: '{a|{b}}{abg|}\n{hr|}\n'+ref.$t('Totalmembership')+'{per|{c}} \n'+ref.$t('Accountedfor')+'{per|{d}%}  ',
                               // formatter:'{b}',
                               // backgroundColor: '#eee',
                              //  borderColor: '#aaa',
                               // borderWidth: 1,
                    ...
                    let incomeCharts = echarts.init(document.getElementById('payUserChannelReport'));
