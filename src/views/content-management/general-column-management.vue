                ...
                <Card>
                    <div slot="title">
                        ...
                        <div style="float:right;">
                            {{$t('nodeType')}}：
                            <Select :placeholder="$t('selectPlaceholder')" v-model="nodeType" style="width:200px;">
                                <Option value="">{{$t('selectPlaceholder')}}</Option>
                                <Option v-for="item in nodeTypeList" :value="item.key" :key="item.key">{{ item.value }}</Option>
                            </Select>
                            <Button type="primary" icon="ios-search" @click="searchCatalogByNodeType">{{$t('search')}}</Button>
                        </div>
                    </div>
                    <div>
            ...
            <Modal  v-model="isShowAddOrEdit" width="800px" >
                ...
                <Card :bordered="false" dis-hover>
                    <Form ref="catalog" :model="catalog" :label-width="180">
                        ...
                        <FormItem :label="$t('nodeType')" class='ivu-form-item-required' v-show="defaultLanguage==languageConstant">
                            <Select class="catalog_field" :placeholder="$t('selectPlaceholder')" v-model="catalog.nodeType" @on-change="nodeTypeChange" :disabled="isShowEdit" >
                                <Option v-for="item in nodeTypeList" :value="item.key" :key="item.key">{{ item.value }}</Option>
                            </Select>
                            <div class="ivu-form-item-error-tip" v-text="nodeTypMsg" v-show='showMsg'></div>
                        </FormItem>

                        <FormItem :label="$t('parentNode')" v-show="defaultLanguage==languageConstant && catalog.nodeType == 1" 
                            class='ivu-form-item-required' >
                            <Select class="catalog_field" v-model="catalog.parentCatalogId" :placeholder="$t('selectPlaceholder')" :disabled="isShowEdit" :loading="isLoadingParentNode" filterable @on-change="validParentCatalogFun()">
                                <Option v-for="item in parentNode" :value="item.catalogId" :key="item.catalogId">{{ item.catalogName }}</Option>
                            </Select>
                            <div class="ivu-form-item-error-tip" v-text="parentCatalogMsg" v-show='showMsg'></div>
                        </FormItem>
                        <FormItem :label="$t('catalogIconCol')" v-show="defaultLanguage==languageConstant" 
                            :class="{'ivu-form-item-required':showRequired}">
...
export default {
    name: 'generalColumnManagement',
    data () {
        return {
            fileUrl: util.fileHeaherUrl,
            nodeType: null, //节点类型检索条件
            pageIndex: 1,   //查询页码
            pageSize: 10,   //查询大小
            total: 0, //总数  
            loadingTable: true,   //表格是否loading
            isLoadingParentNode: false,    //是否loading父栏目下拉框
            ...
            nodeTypeList: [
                {
                    key: 1,
                    value: this.$t('leafNode')
                },
                {
                    key: 2,
                    value: this.$t('notLeafNode')
                }
            ],
            columns: [
                {
                ...
                {
                    title: this.$t('nodeType'),
                    key: 'nodeType',
                    align:'center',
                    render: (h, params) => {
                        var nodeType = "";
                        if(params.row.nodeType == 1){
                            nodeType = this.$t('leafNode');
                        }else if(params.row.nodeType == 2){
                            nodeType = this.$t('notLeafNode');
                        }
                        return h('span',nodeType);
                    }
                },
                {
                ...
                /*{
                    title: this.$t('parentCatalogNameCol'),
    ...
    methods: {
        ...
        nodeTypeChange(){
            if(this.catalog.nodeType == 2){
                this.catalog.parentCatalogId = '';
                this.showRequired=true;
            }
            if(this.catalog.nodeType == 1){
                this.isLoadingParentNode = true;
                ...
                this.$http.post(url, params).then(
                    function(res){
                        ...
                        if(!!res && res.resultCode == '0'){
                            ...
                            if(!!catalogList){
                                for(var i=0;i<catalogList.length;i++){
                                    ref.parentNode.push(catalogList[i]);
                                }
                            }
                        }
                        ref.isLoadingParentNode = false;
                    }
                );
            }
            this.validNodeTypeFun();
        },
        showAdd(){
            this.initCatalog();
                ...
                function(res){
                    if(!!res && res.resultCode == '0'){
                        ...
                        if(!!catalogData){
                            ...
                            ref.catalog.nodeType = catalogData.nodeType;
                            ref.catalog.parentCatalogId = catalogData.parentCatalogId;
        ...
        edit(){    
            ...
            //判断是否存在于缓存数组
            for (var ii2 = 0; ii2 < this.waitIconTemp.length; ii2++){ 
        　　    if(this.waitIconTemp[ii2] == catalogDto.catalogIcon){
                    this.waitIconTemp.splice(ii2,1);
                    break;
                }   
            } 
       ...
       //清空catalog图片信息
        ...
        validNodeTypeFun () {
            ...
            if(this.defaultLanguage==this.languageConstant){   
                if (!!!this.catalog.nodeType) {
                    this.nodeTypMsg=this.$t('catalogNodeTypeWarning');
                }
            }
        },
        validParentCatalogFun () {
            this.parentCatalogMsg='';
            if(this.defaultLanguage==this.languageConstant){  
                if (this.catalog.nodeType == 1 && !!!this.catalog.parentCatalogId) {
                    this.parentCatalogMsg=this.$t('parentCatalogWarning');
                }
            }
        },
        validCatalogIconFun () {
        ...
        vaildCatalog(){
            if(this.defaultLanguage==this.languageConstant){
                ...
                if (this.catalog.nodeType == 2 && !!!this.catalog.catalogIcon ) {
                    this.catalogIconMsg=this.$t('catalogIconWarning');
                }
            } 
            if (!!!this.catalog.catalogName) {
            ...
            if(!!this.nodeTypMsg ||!!this.parentCatalogMsg||!!this.catalogIconMsg||!!this.catalogNameMsg||!!this.progagandaImgMsg)
            {
                this.showMsg=true;
                return true;
            }else
        ...
        vaildCatalogInfo(langName,cataloglang){
            ...
            if(langName==this.languageConstant){
                if(!!!cataloglang.nodeType){
                    return false;
                }
                if(cataloglang.nodeType==1 && !!!cataloglang.parentCatalogId){
                    return false;
                }
                if(cataloglang.nodeType==2 && !!!cataloglang.catalogIcon){
                    return false;
                }
                return true;
            }
