                ...
                <Card> 
                    <div slot="title">                        
                        <template>
                            ...
                            <div class="lineBlock">
                                <span>&nbsp;{{$t('catalogNameCol')}}：</span>
                                <Input v-model="serchKey" :placeholder="$t('searchholder')" clearable style="width: 200px;">
                                </Input>
                                <Button type="primary" icon="ios-search" @click="searchCatalogByName">{{$t('search')}}</Button>
                            </div>
                        </template>
                    </div>
                    <div>
        ...
        <Row>
            ...
            <Modal v-model="isShowAddOrEdit" width="800px">
                ...
                <Card :bordered="false" dis-hover>
                    <Form ref="catalog" :model="catalog" :label-width="180" class='ivu-form-item-required'>
...
export default {
    ...
    methods: 
    {
        ...
        showEdit(catalogId,index)
        {
            ...
            this.waitPorgTemp=[]; 
            this.isShowAdd = false; 
            this.isLoadingEdits.splice(index,1,true);  // 设置按钮加载样式

            let ref = this;   
            let catalogIdtemp = catalogId;
            ...
            this.$http.post(url,params).then(
                function(res)
                {
                    ...
                    ref.isShowAddOrEdit = true;
                    ref.isShowEdit = true;
                    ref.isLoadingEdits.splice(index,1,false);  // 设置按钮加载样式
                }
            );
        },
        cancelEdit() 
        ...
        removeCatalog(catalog)
        {
            ...
            this.$Modal.confirm({
                ...
                title: ref.$t('deleteConfirmationTitle'),
                content: '<p>'+ref.$t('deleteConfirmContent')+'</p>',
                loading: true,
                okText: ref.$t('confirmBtn'),
                cancelText: ref.$t('cancelBtn'),
                onOk: () => {
                    this.$http.post(byIdUrl, param).then(function(resp){
                        if(!!resp && resp.resultCode == '0'){
                            var catalogData = resp.data;
                            ref.$http.post(url,param).then(function(res){
                                if(!!res && res.resultCode === '0'){
                                    ref.$Modal.remove();
       ...
       //清空catalog图片信息
        ...
        vaildCatalog(){
            ...
            if(!!this.catalogIconMsg||!!this.catalogNameMsg||!!this.progagandaImgMsg)
            {
                this.showMsg=true;
                return true;
            }else
